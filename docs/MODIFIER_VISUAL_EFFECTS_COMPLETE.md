...
```vue
<!-- 简单易用的 API -->
<ModifiedValue :value="120" :attribute-info="atkModifierInfo" size="md" inline />
```
...
const getSkillModifierInfo = (skill: SkillMessage, attributeName: string) => {
  ...
  const possibleNames = [`skill_${skill.id}_${attributeName}`, `skill_${skill.baseId}_${attributeName}`, attributeName]

  return activePet.modifierState.attributes.find(attr => possibleNames.includes(attr.attributeName))
}
```
