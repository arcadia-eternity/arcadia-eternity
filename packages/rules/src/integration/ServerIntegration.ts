...
export class ServerRuleIntegration {
  ...
  private static dataRepository: Record<string, unknown> | null = null
   ...
   */
  static async initializeServer(dataRepository?: Record<string, unknown>): Promise<void> {
    // 保存数据仓库以供后续自动初始化使用
    if (dataRepository) {
   ...
   */
  static initializeSpeciesDataProvider(dataRepository: Record<string, unknown>): void {
