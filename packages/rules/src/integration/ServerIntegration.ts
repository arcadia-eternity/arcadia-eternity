...
export class ServerRuleIntegration {
  ...
  private static dataRepository: unknown = null
   ...
   */
  static async initializeServer(dataRepository?: unknown): Promise<void> {
    // 保存数据仓库以供后续自动初始化使用
    if (dataRepository) {
   ...
   */
  static initializeSpeciesDataProvider(dataRepository: unknown): void {
