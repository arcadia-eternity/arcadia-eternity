  ...
  async execute(world: BattleWorld, phase: PhaseDef, bus: EventBus): Promise<PhaseResult> {
    ...
    try {
      ...
      if (!ctx.hitResult) {
        ...
        await this.applyDefeatIfNeeded(world, bus, ctx, contextEntityId)
        setComponent(world, contextEntityId, 'context', ctx)
      ...
      setComponent(world, contextEntityId, 'context', ctx)
      while (ctx.currentHitCount <= hitCount) {
        ...
        if (ctx.category !== Category.Status) {
          ...
          await this.phaseManager.execute(world, this.phaseManager.getHandler(Phase.damage)!, bus, {
            context: damageCtx,
          })
