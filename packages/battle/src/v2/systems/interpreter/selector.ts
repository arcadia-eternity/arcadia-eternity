...
export function applyChain(ctx: InterpreterContext, results: unknown[], chain: SelectorChain[]): unknown[] {
  ...
  const resolveVirtualContextValue = (obj: Record<string, unknown>, key: string): unknown => {
    // Generic virtual fields that are not bound to a specific context type.
    if (key === 'effect') {
    ...
    case 'switch-pet':
      if (key === 'switchInPet') return obj.switchInPetId
      if (key === 'switchIn') return obj.switchInPetId
  ...
  const resolvePathWithContextAliases = (target: unknown, path: string): unknown => {
    ...
    for (const part of parts) {
      ...
      const virtualValue = resolveVirtualContextValue(obj, part)
  ...
  for (const step of chain) {
    ...
    switch (s.type) {
      case 'selectPath': {
        ...
        current = current
          .flatMap(item => {
            const resolved = resolvePathWithContextAliases(item, path)
            if (resolved !== undefined) return [resolved]
            // v2 selectors often carry entity IDs; allow selectPath to read
            // entity attributes/relations via extractor fallback.
      ...
      case 'selectObservable': {
        // v1 observable selector: in v2 we resolve current scalar value by path
        const path = s.arg
        current = current
          .map(item => {
        ...
        current = current
          .map(item => {
            if (typeof item === 'string') {
              return attrSystem.getValue(ctx.world, item, key)
            }
            if (typeof item === 'object' && item !== null) {
              const obj = item as Record<string, unknown>
              return obj[key]
      ...
      case 'asStatLevelMark': {
        // Filter to only stat level marks.
        // Supports both:
        // - mark entity ID (string) with statStage tag
        // - inline/base mark object from addMarkContext.baseMark (id: stat_stage_*)
        const { markSystem } = ctx.systems
        current = current.filter(item => {
          if (typeof item === 'string') {
            return markSystem.getTags(ctx.world, item).includes('statStage')
          }
          if (typeof item === 'object' && item !== null) {
            const obj = item as Record<string, unknown>
      ...
      case 'sampleBetween': {
        // Sample a random value between first and second element
        ...
        const rng = ctx.systems.rng
        const randomValue = rng.nextInt(firstVal, secondVal + 1)
        current = [randomValue]
        break
      }
      ...
      default:
        current = applyCommonSelectorChain(current, [s as CommonSelectorChainStep], {
          resolveValue: value => resolveValue(ctx, value as Value | null | undefined),
...
function applyExtractor(ctx: InterpreterContext, entityId: unknown, extractor: unknown): unknown[] {
  ...
  const getByPath = (target: unknown, path: string): unknown => {
    ...
    for (const part of parts) {
      ...
      // Virtual fields for legacy DSL compatibility (skill.id / skill.base / effect.id / etc.)
      if (part === 'effect') {
        const effectObject = obj.effect
        if (typeof effectObject === 'object' && effectObject !== null) {
          current = effectObject
          continue
        }
        const effectId = typeof obj.effectId === 'string' ? obj.effectId : undefined
        if (effectId) {
      ...
      if (type === 'use-skill') {
        if (part === 'skill') {
          const skillId = typeof obj.skillId === 'string' ? obj.skillId : undefined
          if (!skillId) return undefined
          current = skillSystem.get(world, skillId) ?? { id: skillId }
          continue
        }
        if (part === 'pet') {
          const petId = typeof obj.petId === 'string' ? obj.petId : undefined
          if (!petId) return undefined
          current = petSystem.get(world, petId) ?? { id: petId }
          continue
        }
        if (part === 'source') {
          current = obj.petId
      ...
      if (type === 'skill' && part === 'base') {
        const baseSkillId = typeof obj.baseSkillId === 'string' ? obj.baseSkillId : undefined
        if (!baseSkillId) return undefined
        current = getComponent(world, baseSkillId, 'baseSkill') ?? { id: baseSkillId }
        continue
      }
      if (type === 'skill' && part === 'baseId') {
        const baseSkillId = typeof obj.baseSkillId === 'string' ? obj.baseSkillId : undefined
        if (!baseSkillId) return undefined
        current = baseSkillId
        continue
      }
      if (type === 'mark' && part === 'base') {
        const baseMarkId = typeof obj.baseMarkId === 'string' ? obj.baseMarkId : undefined
        if (!baseMarkId) return undefined
        current = getComponent(world, baseMarkId, 'baseMark') ?? { id: baseMarkId }
        continue
      }
      if (type === 'mark' && part === 'baseId') {
        const baseMarkId = typeof obj.baseMarkId === 'string' ? obj.baseMarkId : undefined
        if (!baseMarkId) return undefined
        current = baseMarkId
        continue
      }
      if (type === 'pet' && part === 'lastSkill') {
        const lastSkillId = typeof obj.lastSkillId === 'string' ? obj.lastSkillId : undefined
        if (!lastSkillId) return undefined
        current = skillSystem.get(world, lastSkillId) ?? { id: lastSkillId }
        continue
      }
  ...
  // Legacy DSL compatibility: `stat.xxx` means reading runtime attribute `xxx` from attrSystem.
  if (extractorKey.startsWith('stat.')) {
    const statKey = extractorKey.slice('stat.'.length)
    if (!statKey) return []
    const resolved = resolveExtractorByKind(world, systems, entityId, 'attribute', statKey)
    if (resolved.length > 0) return resolved
    if (!isObjectEntity) return []
    const statBag = getByPath(entityId, 'stat')
    const fromBag = statBag !== undefined ? getByPath(statBag, statKey) : undefined
    if (fromBag !== undefined) return [fromBag]
    const fromDirect = getByPath(entityId, statKey)
    return fromDirect !== undefined ? [fromDirect] : []
  }
  ...
  switch (extractorKey) {
    ...
    case 'element': {
      if (!isStringEntity) {
        const value = isObjectEntity ? getByPath(entityId, extractorKey) : undefined
        return value !== undefined ? [value] : []
      }
      const pet = petSystem.get(world, entityId)
      return pet ? [petSystem.getElement(world, entityId)] : []
    }
