...
import eloRoutes from '../../api/routes/elo'
import ruleSetsRoutes from '../../api/routes/rulesets'
...
export function createClusterApp(config: Partial<ClusterServerConfig> = {}): {
  ...
  const start = async (): Promise<void> => {
    ...
    try {
      ...
      const { createClusterAuthService } = await import('../../domain/auth/services/clusterAuthService')
