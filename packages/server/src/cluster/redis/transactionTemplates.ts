...
export class TransactionTemplates {
   ...
   * 实例注册事务
   */
  async executeInstanceRegistration(instanceId: string, instanceData: Record<string, string>): Promise<boolean> {
    try {
      const operations: TransactionOperation[] = [
        {
          type: 'sadd',
  ...
  private serializeSessionConnection(connection: Record<string, string>): Record<string, string> {
    return {
