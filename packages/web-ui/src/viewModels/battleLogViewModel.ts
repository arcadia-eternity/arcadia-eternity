...
export type FormattedBattleMessage = BattleMessage & {
  icon: string
  content: string
  timestamp: string
}
...
export function useBattleLogViewModel(
  ...
  const petMap = computed(() => petData || new Map<string, PetMessage>())
  const skillMap = computed(() => skillData || new Map<string, SkillMessage>())
  const playerMap = computed(() => playerData || new Map<string, PlayerMessage>())
  const markMap = computed(() => markData || new Map<string, MarkMessage>())

  // 格式化消息数据 - 直接复用 formatBattleMessage 函数
  const formattedMessages = computed(() => {
    return messages.map(msg => formatBattleMessage(msg, petMap.value, skillMap.value, playerMap.value, markMap.value))
  })

  // 获取格式化后的单个消息 - 直接复用 formatBattleMessage
  const getFormattedMessage = (msg: BattleMessage) => {
    return formatBattleMessage(msg, petMap.value, skillMap.value, playerMap.value, markMap.value)
  }
  ...
  return {
    formattedMessages,
    getFormattedMessage,
    messageIcons: MESSAGE_ICONS,
    damageTypeMap: DAMAGE_TYPE_MAP,
    statArrows: getStatArrows,
    logContainerRef,
    clearMessages,
  }
}
