    ...
    >
      <div class="version-details space-y-3">
        ...
        <div class="flex justify-between items-center">
          ...
          <el-tag 
            :type="versionInfo.environment === 'production' ? 'success' : 'warning'"
            size="small"
          >
        ...
        <div class="flex justify-between items-center">
          ...
          <el-tag 
            :type="versionInfo.isTauri ? 'primary' : 'info'"
            size="small"
          >
        ...
        <div 
          v-if="versionInfo.environment === 'development'"
      ...
      <template #footer>
        <el-button @click="showDetails = false" size="small">
          关闭
        </el-button>
        <el-button 
...
import { getVersionInfo, getVersionString, getDetailedVersionString } from '@/utils/version'

// 版本信息
const versionInfo = getVersionInfo()
const versionString = getVersionString()
const detailedVersionString = getDetailedVersionString()
...
const buildTimeFormatted = computed(() => {
  return new Date(versionInfo.buildTime).toLocaleString('zh-CN', {
    year: 'numeric',
    month: '2-digit',
    day: '2-digit',
    hour: '2-digit',
...
async function copyVersionInfo() {
  try {
    await navigator.clipboard.writeText(detailedVersionString)
