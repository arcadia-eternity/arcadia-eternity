      ...
      <div v-else-if="skill" class="space-y-8">
        ...
        <div class="bg-slate-800/40 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
          <div class="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
            <!-- 技能图标区域 -->
            <div class="flex-shrink-0 flex items-center justify-center w-32 h-32 bg-slate-700/50 rounded-lg">
            ...
            <div class="flex-1 text-center md:text-left">
              ...
              <div class="flex items-center justify-center md:justify-start space-x-4 mb-4">
                ...
                <span class="text-gray-300">{{ getCategoryName() }}</span>
              </div>
              <div class="text-gray-400" v-html="getRenderedSkillDescription()"></div>
            </div>
          </div>
        </div>
        ...
        <div class="bg-slate-800/40 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-slate-700/30 rounded-lg p-3">
              <div class="text-gray-400 text-sm">{{ i18next.t('dex.skillDetail.power', { ns: 'webui' }) }}</div>
              <div class="text-white font-bold text-lg">{{ skill.power || '-' }}</div>
            </div>

            <!-- 命中率 -->
            <div class="bg-slate-700/30 rounded-lg p-3">
              <div class="text-gray-400 text-sm">{{ i18next.t('dex.skillDetail.accuracy', { ns: 'webui' }) }}</div>
              <div class="text-white font-bold text-lg">{{ skill.accuracy }}%</div>
            </div>

            <!-- 怒气消耗 -->
            <div class="bg-slate-700/30 rounded-lg p-3">
              <div class="text-gray-400 text-sm">{{ i18next.t('dex.skillDetail.rage', { ns: 'webui' }) }}</div>
              <div class="text-white font-bold text-lg">{{ skill.rage }}</div>
            </div>

            <!-- 优先度 -->
            <div class="bg-slate-700/30 rounded-lg p-3">
              <div class="text-gray-400 text-sm">{{ i18next.t('dex.skillDetail.priority', { ns: 'webui' }) }}</div>
              <div class="text-white font-bold text-lg">{{ skill.priority || 0 }}</div>
            </div>
          </div>
        </div>
        ...
        <!-- 技能特性 -->
        <div class="bg-slate-800/40 backdrop-blur-sm border border-slate-700 rounded-lg p-6">
          <h2 class="text-xl font-bold text-white mb-4">技能特性</h2>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- 特殊效果 -->
            <div v-if="hasSpecialEffects" class="bg-slate-700/30 rounded-lg p-3 md:col-span-2">
        ...
        >
          ...
          <div class="space-y-2">
            <div v-for="effectId in skill.effect" :key="effectId" class="bg-slate-700/30 rounded-lg p-3">
