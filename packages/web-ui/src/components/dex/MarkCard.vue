  ...
  >
    ...
    <div class="flex items-center gap-3 mb-3" :class="{ 'mb-2': compact }">
      ...
      >
        ...
        <ShieldCheckIcon v-else class="text-purple-400" :class="compact ? 'w-6 h-6' : 'w-8 h-8'" />
      </div>
      ...
      <div class="flex-1 min-w-0">
        ...
        <div class="text-gray-400 truncate" :class="compact ? 'text-xs' : 'text-sm'">
          {{ markId }}
        </div>
      </div>
    </div>
    ...
    <!-- 印记描述 (非紧凑模式) -->
    <div v-if="!compact && markDescription" class="text-xs text-gray-300 mb-3 line-clamp-2 leading-relaxed">
      {{ markDescription }}
    </div>
    ...
    <div v-if="relation && showRelationType" class="space-y-2">
      ...
      <div class="flex items-center gap-2">
        <component :is="getRelationTypeIcon(relation.relationType)" class="w-4 h-4" />
        <span class="text-xs px-2 py-1 rounded border" :class="getRelationTypeClass(relation.relationType)">
      ...
      <div class="flex items-center gap-1 text-xs text-gray-500">
        ...
        <span v-if="relation.effectId" class="text-gray-400"> ({{ relation.effectId }}) </span>
      </div>
    </div>

    <!-- 印记配置信息 (紧凑模式下的额外信息) -->
    <div v-if="compact && mark" class="mt-2 flex flex-wrap gap-1">
      <span v-if="mark.config?.stackable" class="text-xs bg-blue-600/20 text-blue-300 px-1.5 py-0.5 rounded">
        可叠加
      </span>
      <span v-if="mark.config?.persistent" class="text-xs bg-green-600/20 text-green-300 px-1.5 py-0.5 rounded">
        持续
      </span>
      <span
